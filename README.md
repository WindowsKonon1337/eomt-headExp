...
This repository now uses [uv](https://github.com/astral-sh/uv) as the dependency manager.
...
To train EoMT from scratch locally (without Docker), run:

```bash
./scripts/train_local.sh \
  -c configs/dinov2/coco/panoptic/eomt_large_640.yaml \
  --trainer.devices 4 --data.batch_size 4 --data.path /path/to/dataset
```
...
CONFIG_PATH=configs/dinov2/coco/panoptic/eomt_large_640.yaml \
TRAINER_DEVICES=4 \
BATCH_SIZE=4 \
...
uv run python main.py fit \
  -c configs/dinov2/coco/panoptic/eomt_base_640_cnn.yaml \
...
```bash
uv run python main.py fit \
  -c configs/dinov2/coco/panoptic/eomt_base_640_kan.yaml \
  --data.path /path/to/dataset
```

You can also switch from CLI:

```bash
--model.network.mask_head_type kan --model.network.kan_basis_size 8
```
